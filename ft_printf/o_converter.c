...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/12/14 11:41:30 by vdubinki          #+#    #+#             */
/*   Updated: 2017/12/14 11:41:32 by vdubinki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
												...
												t_options *options)
{
	*pn = 0;
	*flag = 0;
	if (options->flags[4] && !options->flags[2] && !*diff)
		*flag = 1;
}
								...
								t_options *options)
{
	...
	arg = ft_itoabase(get_arg_by_type(ap, length_modifier), 8, 0);
	if (!ft_strcmp(arg, "0"))
		if (options->precision == 0 || options->flags[4])
			arg = "";
	len = ft_strlen(arg);
	diff = 0;
...
{
	length_modifier = 3;
	return (o_converter(ap, length_modifier, options));
}
